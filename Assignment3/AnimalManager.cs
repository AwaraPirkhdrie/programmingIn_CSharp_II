...
namespace Animal_Park
{
    ...
    class AnimalManager: ListManager<Animal>
    {
        ...
        /// <summary>
        /// Deletes animal from food-animal dictionary
        /// </summary>
        /// <param name="anim_id"></param>
        public void Delete_animal_from_food_dictionary(int anim_id)
        {
            int key_val = FoodItemConnected(anim_id);

            if (key_val < 0)
            {
                return;
            }
            int[] test_array = this.Dict_AnimalFood[key_val];
            int[] new_array = test_array.Where(e => e != anim_id).ToArray();

            this.Dict_AnimalFood[key_val] = new_array;
        }
        /// <summary>
